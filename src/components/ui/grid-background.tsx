...
function GridBackground({
    ...
    const {
        background = 'bg-transparent', // Changed default to transparent to blend with footer
    ...
    const {
        count = 12,
        colors: beamColors = DEFAULT_BEAM_COLORS,
    ...
    const [animatedBeams, setAnimatedBeams] = React.useState<Array<{
        id: number;
        color: string;
    ...
    React.useEffect(() => {
        const beams = Array.from({ length: Math.min(count, 12) }, (_, i) => {
            const direction = (Math.random() > 0.5 ? 'horizontal' : 'vertical') as 'horizontal' | 'vertical';
            const startPosition = (Math.random() > 0.5 ? 'start' : 'end') as 'start' | 'end';
